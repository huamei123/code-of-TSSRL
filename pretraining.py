...
def TSSRL_pretrain(model,cat_idxs,X_train,y_train,continuous_mean_std,opt,device):
    train_ds = DataSetCatCon(X_train, y_train, cat_idxs,opt.dtask, continuous_mean_std)
    trainloader = DataLoader(train_ds, batch_size=opt.batchsize, shuffle=True,num_workers=0)
    vision_dset = opt.vision_dset
    # optimizer = optim.AdamW(model.parameters(),lr=0.0001)
    # optimizer = optim.LARS(model.parameters(),lr=0.075,momentum=0.9,lamb_weight_decay=0.000001)
    ...
    for epoch in range(opt.pretrain_epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if 'contrastive' in opt.pt_tasks:
                ...
                # targets = torch.ones(logits_per_aug1.size(0)).to(logits_per_aug1.device).long()
